...
int DISTRIBUTE(Location **dataGrid, Cell *actList, unsigned *actCt,
               double *gridmetadata){
/*Module: DISTRIBUTE_06
	args: Global Grid, Active List, Active Count, DEM Metadata
	
	DISTRIBUTE_06 is a BLIND-SHARE Distribution scheme! All lower neighbors are
	              ...
	              This is also an INSTANT-FLOW Distribution scheme! All cells
	              change their lava thicknesses at the same time after flow
	              quantities are calculated.
*/

	unsigned counter = 1;
	unsigned tempActCt = *actCt;
	...
	do { /*for all entries in active list (counter<=actCt)*/
			...
			/*if current cell's thickness > than residual, spread lava*/
				...
				/*If the neighbor list is not empty*/
					...
					for(n=0;n<neigh_ct;n++) {
						...
						actList[dataGrid[nrow][ncol].active].lava_in +=
							     distribute_volume * neigh_diff / total_elev_diff;
							     				
					} /*End for each neighbor*/
				
					actList[counter].lava_out += distribute_volume;
		...
		} while(counter <= tempActCt);
		
		/*Update cell levels*/
		for(n=1;n<=*actCt;n++) {
			actList[n].elev      += actList[n].lava_in;
			actList[n].thickness += actList[n].lava_in;
			
			actList[n].elev      -= actList[n].lava_out;
			actList[n].thickness -= actList[n].lava_out;
			
			actList[n].lava_in = actList[n].lava_out = 0.0;
		}
